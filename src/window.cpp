...
PicoVimWindow::PicoVimWindow(PicoVimDisplay& display)
    : PicoVimComponent(display.GetEditor()),
    m_display(display)
{
}

PicoVimWindow::~PicoVimWindow()
{
}

void PicoVimWindow::SetCurrentBuffer(PicoVimBuffer* pBuffer)
{
    m_buffers.insert(pBuffer);
    m_pCurrentBuffer = pBuffer;
}

PicoVimBuffer* PicoVimWindow::GetCurrentBuffer() const
{
    return m_pCurrentBuffer;
}

void PicoVimWindow::AddBuffer(PicoVimBuffer* pBuffer)
{
    m_buffers.insert(pBuffer);
...
void PicoVimWindow::RemoveBuffer(PicoVimBuffer* pBuffer)
{
    ...
    if (m_pCurrentBuffer == pBuffer)
    {
        ...
        if (!m_buffers.empty())
        {
            m_pCurrentBuffer = *m_buffers.begin();
        }
    }

}

const PicoVimWindow::tBuffers& PicoVimWindow::GetBuffers() const
{
    return m_buffers;
}

void PicoVimWindow::SetCursorMode(CursorMode mode)
{
    cursorMode = mode;
    m_display.ResetCursorTimer();
}

void PicoVimWindow::Notify(std::shared_ptr<PicoVimMessage> payload)
{
...
long PicoVimWindow::ClampVisibleLine(long line) const
{
...
NVec2i PicoVimWindow::ClampVisibleColumn(NVec2i pos, LineLocation location) const
{
...
BufferLocation PicoVimWindow::DisplayToBuffer() const
{
    return DisplayToBuffer(cursorCL);
}

// Convert a display coordinate to a buffer coordinate
BufferLocation PicoVimWindow::DisplayToBuffer(const NVec2i& display) const
{
...
NVec2i PicoVimWindow::BufferToDisplay(const BufferLocation& loc) const
{
    NVec2i ret(0, 0);
    if (visibleLines.empty())
    {
...
void PicoVimWindow::MoveCursor(LineLocation location)
{
...
void PicoVimWindow::MoveCursorTo(const BufferLocation& location, LineLocation clampLocation)
{
    MoveCursor(BufferToDisplay(location) - cursorCL, clampLocation);
}

void PicoVimWindow::MoveCursor(const NVec2i& distance, LineLocation clampLocation)
{
...
void PicoVimWindow::SetSelectionRange(const NVec2i& start, const NVec2i& end)
{
...
void PicoVimWindow::SetStatusText(const std::string& strText)
{
    statusLines = StringUtils::SplitLines(strText);
}

void PicoVimWindow::ClampCursorToDisplay()
{
...
void PicoVimWindow::PreDisplay(const DisplayRegion& region)
{
...
bool PicoVimWindow::DisplayLine(const LineInfo& lineInfo, const DisplayRegion& region, int displayPass)
{
