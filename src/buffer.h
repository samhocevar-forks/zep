...
struct BufferMessage : public PicoVimMessage
{
    BufferMessage(PicoVimBuffer* pBuff, BufferMessageType messageType, const BufferLocation& startLoc, const BufferLocation& endLoc, const BufferLocation& cursor = BufferLocation{ -1 })
        : PicoVimMessage(Msg_Buffer),
        pBuffer(pBuff),
        type(messageType),
...
class PicoVimBuffer : public PicoVimComponent
{
public:
    PicoVimBuffer(PicoVimEditor& editor, const std::string& strName);
    virtual ~PicoVimBuffer();
    ...
    long ClampLine(long line) const;
    ...
    void SetSyntax(std::shared_ptr<PicoVimSyntax> spSyntax) { m_spSyntax = spSyntax; }
    PicoVimSyntax* GetSyntax() const { return m_spSyntax.get(); }
