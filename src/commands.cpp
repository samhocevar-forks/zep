#include "commands.h"

namespace PicoVim
{

// Delete Range of chars
PicoVimCommand_DeleteRange::PicoVimCommand_DeleteRange(PicoVimBuffer& buffer, const BufferLocation& start, const BufferLocation& end, const BufferLocation& cursorAfter)
    : PicoVimCommand(buffer),
    m_startOffset(start),
    m_endOffset(end),
    m_cursorAfter(cursorAfter)
{
    // We never allow deletion of the '0' at the end of the buffer
    if (buffer.GetText().empty())
    {
        m_endOffset = m_startOffset;
    }
    else
    {
        m_endOffset = std::min(m_endOffset, long(buffer.GetText().size()) - 1l);
    }
}

void PicoVimCommand_DeleteRange::Redo()
{
    if (m_startOffset != m_endOffset)
    {
        m_deleted = std::string(m_buffer.GetText().begin() + m_startOffset,
            m_buffer.GetText().begin() + m_endOffset);

        m_buffer.Delete(m_startOffset, m_endOffset, m_cursorAfter);
    }
}

void PicoVimCommand_DeleteRange::Undo()
{
    if (m_deleted.empty())
        return;
    m_buffer.Insert(m_startOffset, m_deleted, m_startOffset);
}

// Insert a string
PicoVimCommand_Insert::PicoVimCommand_Insert(PicoVimBuffer& buffer, const BufferLocation& start, const std::string& str, const BufferLocation& cursorAfter)
    : PicoVimCommand(buffer),
    m_startOffset(start),
    m_strInsert(str),
    m_cursorAfter(cursorAfter)
{
    m_startOffset = buffer.Clamp(m_startOffset);
}

void PicoVimCommand_Insert::Redo()
{
    bool ret = m_buffer.Insert(m_startOffset, m_strInsert, m_cursorAfter);
    assert(ret);
    if (ret == true)
    {
        m_endOffsetInserted = m_buffer.LocationFromOffset(m_startOffset + long(m_strInsert.size()));
    }
    else
    {
        m_endOffsetInserted = -1;
    }
}

void PicoVimCommand_Insert::Undo()
{
    if (m_endOffsetInserted != -1)
    {
        m_buffer.Delete(m_startOffset, m_endOffsetInserted, m_startOffset);
    }
}
}